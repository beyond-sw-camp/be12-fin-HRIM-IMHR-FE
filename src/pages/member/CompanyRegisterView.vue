...
<template>
      ...
      <div class="w-full max-w-sm p-6 border border-gray-200 rounded shadow-md">
        ...
        <form @submit.prevent="submit" class="space-y-3">
          <input
            v-model="form.name"
            type="text"
            placeholder="초기 관리자 이름"
            class="w-full px-4 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-slate-500"
          />
          <input
            v-model="form.memberId"
            type="text"
            placeholder="초기 관리자 아이디"
            class="w-full px-4 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-slate-500"
          />
          <input
            v-model="form.email"
            type="email"
            placeholder="초기 관리자 이메일"
            class="w-full px-4 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-slate-500"
          />
          <input
            v-model="form.password"
            type="password"
            placeholder="초기 관리자 비밀번호"
            class="w-full px-4 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-slate-500"
          />
          <input
  ...
  >
    <div class="bg-white rounded-lg p-6 w-[500px] shadow-lg relative">
      <h3 class="text-center font-semibold text-lg mb-4">
        사업자등록증명원을 첨부해주세요
      </h3>
      ...
      <div class="flex justify-center items-center gap-6 mb-4">
        <img
          src="/sample-doc-ok.png"
          alt="정상 증명서"
          class="w-24 h-32 object-contain"
        />
        <img
          src="/sample-doc-fail.png"
          alt="불량 증명서"
          class="w-24 h-32 object-contain"
        />
      </div>
      ...
      <button
        ...
        class="w-full bg-slate-800 text-white py-2 rounded hover:bg-slate-900 transition font-semibold mt-2"
      >
        파일 선택
      </button>
      ...
      <button
        ...
        class="mt-3 w-full bg-slate-800 text-white py-2 rounded hover:bg-slate-900 transition font-semibold "
      >
        확인
      </button>
      ...
      <button
        @click="isModalOpen = false"
        class="absolute top-2 right-3 text-slate-500 hover:text-black text-lg"
      >
        &times;
      </button>
    </div>
